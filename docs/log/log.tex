...
\begin{document}
	...
	\begin{itemize}
		\item opencv 4.x, a library aimed at computer vision;
		\item imutils, series of convenience functions to make basic image processing functions such as translation, rotation, resizing, skeletonization;
		\item sklearn, a machine learning library, featuring various classification, regression and clustering algorithms including support vector machines, random forests, gradient boosting, k-means and DBSCAN;
		\item numpy, a library that adds support for large, multi-dimensional arrays and matrices, along with a large collection of high-level mathematical functions to operate on these arrays;
		\item matplotlib, a comprehensive library for creating static, animated, and interactive visualizations;
	\end{itemize}
	...
	\section{SLAM}
	We decided to use gMapping, one of the most used SLAM algorithms\cite{gmapping}.
	\begin{itemize}
		\item Gaussian (EKF) approximation of odometry model from Probabilistic Robotics
		\item Discrete time steps (=when updates happen) correspond to whenever the robot has traveled
	\end{itemize}
