...
\begin{document}
	...
	di evitarne l'esposizione, oltre all' utilizzo di mascherine e dispositivi di protezione personale e la frequente igienizzazione delle mani. Il distanziamento sociale si configura di
	...
	spazi chiusi. In tal senso l'utilizzo di un robot evita l'inutile esposizione da parte di un operatore all'agente patogeno. 
	...
	configurare l'ambiente ROS ed inoltre per capire i concetti fondamentali
	relativi ai nodi e topics. Infine abbiamo impostato l'interfaccia ROS su
	Webots seguendo la documentazione cyberbotics rilevante
	\cite{webotsRosSetup}.
	...
	hardware, il controllo di dei dispositivi a basso livello,
	...
	più macchine) collegati attraverso l'infrastruttura ROS. ROS implementa
	vari stili di comunicazione, fra i quali streaming di dati asincrono
	attraverso i \textbf{topics} e comunicazione sincrona attraverso i
	\textbf{servizi}. 
	...
	Al fine di migliorare la precisione della scansione abbiamo ritenuto opportuno correggere la rotazione del robot con lo spostamento della testa.
	...
	distanza.  Come di evince dalla figura \ref{fig:nms}, la metrica utilizzata
...
\tag{\ref{eq:circular_sector}}
	...
	conseguenza il robot si fermerà in una posizione non ideale. In
	\cite{volpe1990manipulator} viene proposto un modello di potenziale basato
	sulla  \ldots 
