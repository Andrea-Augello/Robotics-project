...
\begin{document}
	...
	L'interpolazione dei campioni e la sovrapposizione parziale del primo e
	l'ultimo campione non permettono di utilizzare una non maxima suppression
	basata sull'indice di Jacquard, rendendo necessario l'uso di un algoritmo
	di clustering.
	...
	massima distanza fra due punti affinché vengano considerati appartenenti a
	un cluster (da non confondere con la massima distanza fra i punti di un
	cluster), \textbf{min\_samples}, ovvero il numero minimo di punti affinché
	un cluster sia valido (nel nostro caso è uguale a 1 in quanto non vogliamo
	...
	distanza fra i due punti ed angolo $\alpha$ corrispondente  all'angolo fra
	i due punti rispetto alla posizione del robot. In questo modo si tiene
	conto sia della distanza orizzontale tra le roi che della loro distanza
	percepita, permettendo di discriminare tra due ROI che ricoprono
	parzialmente un individuo vicino e due roi corrispondenti ad individui
	diversi situati a grande distanza.
