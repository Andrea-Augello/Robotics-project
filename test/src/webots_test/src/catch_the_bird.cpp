...
#include <webots_test/get_float.h>
#include <webots_test/set_float.h>
#include <webots_test/set_int.h>

#include <webots_test/range_finder_get_info.h>
#include <webots_test/robot_get_device_list.h>
#include <webots_test/save_image.h>
...
webots_test::set_int timeStepSrv;
...
int main(int argc, char **argv) {
  ...
  ros::ServiceClient deviceListClient =
    n.serviceClient<webots_test::robot_get_device_list>(controllerName + "/robot/get_device_list");
  webots_test::robot_get_device_list deviceListSrv;
  ...
  ros::ServiceClient rangeFinderGetInfoClient =
    n.serviceClient<webots_test::range_finder_get_info>(controllerName + '/' + rangeFinderName + "/get_info");
  webots_test::range_finder_get_info rangeFinderGetInfoSrv;
  ...
  ros::ServiceClient enableRangeFinderClient =
    n.serviceClient<webots_test::set_int>(controllerName + '/' + rangeFinderName + "/enable");
  webots_test::set_int enableRangeFinderSrv;
  ...
  ros::ServiceClient rangeFinderSaveImageClient =
    n.serviceClient<webots_test::save_image>(controllerName + '/' + rangeFinderName + "/save_image");
  webots_test::save_image rangeFinderSaveImageSrv;
  ...
  ros::ServiceClient motorSetPositionClient =
    n.serviceClient<webots_test::set_float>(controllerName + '/' + motorName + "/set_position");
  webots_test::set_float motorSetPositionSrv;
  motorSetPositionSrv.request.value = 0;
  i = 0.2;
  step = 0.025;

  ros::ServiceClient motorGetTargetPositionClient =
    n.serviceClient<webots_test::get_float>(controllerName + '/' + motorName + "/get_target_position");
  webots_test::get_float motorGetTargetPositionSrv;

  // enable time_step
  timeStepClient = n.serviceClient<webots_test::set_int>(controllerName + "/robot/time_step");
