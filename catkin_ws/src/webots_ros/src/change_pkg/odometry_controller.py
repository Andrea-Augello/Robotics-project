...
class Odometry:
    ...
    def gyro_callback(self, values):
        ...
        if self.gyro_values_old.t != 0:
            ...
            odom_angular_velocity= (self.motors.right.velocity - self.motors.left.velocity) / self.WHEEL_DISTANCE 
            ang_vel = odom_angular_velocity if abs((ang_vel - odom_angular_velocity)) < 0.01 else ang_vel
